...
function submit(next: any) {
    ...
    $.ajax({
        ...
        success: function(data) {
            recordReceive({
                response: data, error: null, status: '200', server: $.url().attr().host,
                port: $.url().attr().port,
                why: 'submit'
            })
            console.log(data)
        ...
        error: function(data, status, error) {
            recordReceive({
                response: data, error: error, status: status, server: $.url().attr().host,
                port: $.url().attr().port,
                why: 'submit'
            })
            loading = LoadingState.ready
...
function reload(segmentName: null | string) {
    try {
        ...
        $.when(
            ...
            recordReceive({
                response: [clip_, clipHalf_, ass], error: null, status: '200', server: $.url().attr().host,
                port: $.url().attr().port,
                why: 'reload'
            })
        ...
        recordReceive({
            response: [], error: err, status: '', server: $.url().attr().host,
            port: $.url().attr().port,
            why: 'reload'
        })
        loading = LoadingState.ready
