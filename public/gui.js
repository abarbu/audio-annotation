...
function heightBottom(isReference) {
    if (splitHeight) {
        if (isReference) {
            return '90%';
        }
...
function heightTop(isReference) {
    if (splitHeight) {
        if (isReference) {
            return '100%';
        }
...
function heightText(isReference) {
    if (splitHeight) {
        if (isReference) {
            return '98%';
        }
...
function heightTopLine(isReference) {
    if (splitHeight) {
        if (isReference) {
            return '93%';
        }
...
function add(t1, t2) {
    return lift2(t1, t2, (a, b) => a + b);
}
function sub(t1, t2) {
    return lift2(t1, t2, (a, b) => a - b);
}
function addConst(t, c) {
    return lift(t, a => a + c);
}
function subConst(t, c) {
    return lift(t, a => a - c);
}
function addMin(t1, t2, t3) {
    return lift3(t1, t2, t3, (a, b, c) => Math.min(a + b, c));
}
function subMax(t1, t2, t3) {
    return lift3(t1, t2, t3, (a, b, c) => Math.max(a - b, c));
}
function to(value) {
    return value;
}
...
function lift(value, callback) {
    return callback(value);
}
function lift2(x, y, callback) {
    return callback(x, y);
}
function lift3(x, y, z, callback) {
    return callback(x, y, z);
}
var BufferType;
(function (BufferType) {
    BufferType["normal"] = "normal";
    BufferType["half"] = "half";
})(BufferType || (BufferType = {}));
var DragPosition;
(function (DragPosition) {
    DragPosition["start"] = "startTime";
    DragPosition["end"] = "endTime";
    DragPosition["both"] = "both";
...
function clickOnCanvas() {
    ...
    lastClick = positionToAbsoluteTime(to(mousePosition().x));
    // @ts-ignore
...
function keyboardShortcutsOn() {
    ...
    $(document).bind('keydown', 'shift+right', () => {
        if (selected == null || !isValidAnnotation(annotations[selected])) {
            ...
            annotations[selected].startTime = addMin(annotations[selected].startTime, keyboardShiftOffset, sub(annotations[selected].endTime, keyboardShiftOffset));
            updateWord(annotations[selected]);
        }
    });
    $(document).bind('keydown', 'ctrl+left', () => {
        if (selected == null || !isValidAnnotation(annotations[selected])) {
            ...
            annotations[selected].endTime = subMax(annotations[selected].endTime, keyboardShiftOffset, add(annotations[selected].startTime, keyboardShiftOffset));
            updateWord(annotations[selected]);
        }
    });
...
var buffers = { normal: null, half: null };
var sourceNode;
var javascriptNode;
var startTime = to(0);
var startOffset = to(0);
var lastClick = null;
var selected = null;
var annotations;
var mute = false;
const keyboardShiftOffset = to(0.01);
...
function play(offset_, duration_) {
...
function timeInMovieToPercent(time) {
    return (100 * ((from(time) - startS) / (endS - startS))) + '%';
}
function timeInMovieToTimeInBuffer(time) {
    return positionToTimeInBuffer(absoluteTimeToPosition(time));
}
function absoluteTimeToPosition(time) {
...
function timeInMovieToPosition(time) {
    return to(((from(time) - startS) / (endS - startS)) * canvas.width);
}
...
function redraw(timeOffset) {
    ...
    if (timeOffset != null && from(timeOffset) < endTime) {
        var offset = timeInBufferToPosition(timeOffset);
        ctx.fillStyle = 'rgba(255, 255, 255, 0.9)';
        ctx.fillRect(from(offset), 1, 1, canvas.height);
    }
    if (lastClick != null) {
        ctx.fillStyle = 'rgba(200, 0, 0, 0.9)';
        ctx.fillRect(from(timeInMovieToPosition(lastClick)), 1, 2, canvas.height);
    }
}
...
function updateWords(words) {
    ...
    _.forEach(words, function (word, index) {
        ...
        $('#words')
            .append($('<a href="#">').append($('<span class="word label label-danger">')
            .text(word)
            .data('index', index)))
            .append(' ');
    });
...
function startWord(index, time) {
    if (!_.find(annotations, function (key) {
...
function closestWord(time) {
    return _.sortBy(_.filter(annotations, function (annotation) {
        return annotation.startTime != null && annotation.startTime < time;
    }), function (annotation, _index) {
        return sub(time, annotation.startTime);
    })[0];
}
function endWord(word, time) {
    if (!word) {
    ...
    // TODO Constant
    if (Math.abs(from(sub(time, word.startTime))) < 0.01) {
...
function annotationColor(annotation, isReference) {
    if (isReference)
        return '#5bc0de';
    if (annotation.endTime != null) {
...
function handleClickOnAnnotatedWord(annotation, isReference) {
    return () => {
        // @ts-ignore
...
function handleDragOnAnnotatedWord(annotation, isReference, position) {
    if (!isReference)
        return drag(annotation, position);
    else
        return () => null;
}
function updateWordBySource(annotation, isReference, worker) {
...
function deleteWord(annotation) {
    if (selected != null) {
        if (annotation.startTime != null)
            delete annotation.startTime;
        if (annotation.endTime != null)
            delete annotation.endTime;
...
function fillAnnotationPositions(annotation) {
    if (!annotation.lastClickTimestamp)
        annotation.lastClickTimestamp = -1;
    return annotation;
}
...
function previousAnnotation(index) {
    var word = _.last(_.filter(annotations, function (annotation) {
        return annotation.index < index && annotation.startTime != null;
    }));
    if (word)
        return word.index;
...
function playAnnotation(annotation) {
    ...
    setup(buffers[bufferKind]);
    if (annotation.endTime != null)
        play(timeInMovieToTimeInBuffer(annotation.startTime), sub(timeInMovieToTimeInBuffer(annotation.endTime), timeInMovieToTimeInBuffer(annotation.startTime)));
    else
        play(timeInMovieToTimeInBuffer(annotation.startTime), defaultPlayLength());
}
...
function submit(next) {
    ...
    $.ajax({
        ...
        data: JSON.stringify({
            ...
            annotations: _.map(_.filter(annotations, (a) => !_.isUndefined(a.startTime)), function (a) {
                return {
                    startTime: a.startTime,
                    endTime: a.endTime,
                    index: a.index,
        ...
        success: function (data) {
            ...
            if (data && data.response == 'ok') {
                if (data.stoken != null && token != null) {
                    next();
                    message('success', 'Thanks!<br/>Enter the following two characters back into Amazon Turk: ' +
                        data.stoken);
                }
                else {
                    next();
                ...
                message('danger', 'Failed to submit annotation!<br>Bad server reply!<br/>Please email <a href="mailto:abarbu@csail.mit.edu">abarbu@csail.mit.edu</a> with this message. Your work will not be lost and we will give you credit for it.<br/>' +
                    JSON.stringify([data, annotations]));
            }
        },
        error: function (data, status, error) {
            message('danger', 'Failed to submit annotation!<br>Ajax error communicating with the server!<br/>Please email <a href="mailto:abarbu@csail.mit.edu">abarbu@csail.mit.edu</a> with this message. Your work will not be lost and we will give you credit for it.<br/>' +
                JSON.stringify([data, status, error, annotations]));
        },
    });
}
$('#submit').click((_e) => submit((a) => a));
...
$('#toggle-speed').on('switchChange.bootstrapSwitch', () => {
    stopPlaying();
    if (bufferKind == BufferType.half)
        bufferKind = BufferType.normal;
    else if (bufferKind == BufferType.normal)
        bufferKind = BufferType.half;
});
...
$('#fill-with-reference').click((_e) => {
    ...
    if (referenceAnnotations) {
        ...
        const lastAnnotationEndTime = to(_.max(_.concat(-1, _.map(existingAnnotations, (a) => from(a.endTime)))));
        let mergedAnnotations = _.concat(existingAnnotations, 
        // @ts-ignore
...
javascriptNode.onaudioprocess = function (_audioProcessingEvent) {
    if (sourceNode && audioIsPlaying) {
        if (from(startTime) == -1)
            startTime = to(context.currentTime);
        redraw(to(context.currentTime - from(startTime) + from(startOffset)));
    }
};
...
function register_other_annotations(worker) {
    ...
    if (reference_annotations && worker != parameters.worker) {
        $('#annotations')
            .append($('<button type="button" class="annotation btn btn-default">')
            .text(worker)
            .data('worker', worker)
...
function preloadNextSegment(segment) {
    if (preloadSegments) {
        try {
...
function reload(segmentName) {
    ...
    words = [];
    $('#annotations').empty();
    $('#annotations').append($('<button type="button" class="annotation btn btn-info">')
        .text('none')
    ...
    $.when($.ajax({ url: '/audio-clips/' + movieName + '/' + segment + '.mp3',
        ...
        const ass = ass_[0];
        context.decodeAudioData(clip, function (audioBuffer) {
            buffers['normal'] = audioBuffer;
            setup(buffers['normal']);
        }, onError);
        context.decodeAudioData(clipHalf, function (audioBuffer) {
...
$('#container-wrapper')
    ...
    .attr('data-bootstro-content', "You're going to annotate the beginning and end of each word on this diagram. It's a representation of the audio. Click anyhwere on it to play a chunk of the audio.")
...
$('#toggle-speed-div')
    ...
    .attr('data-bootstro-content', 'It can be hard to catch each word and when it was said. We play the audio at half speed by default, you can change this to regular speed.')
...
$('#spectrogram')
    ...
    .attr('data-bootstro-content', 'Once you have the words, place the red marker on the diagram. A short audio segment will play.')
    .attr('data-bootstro-placement', 'top')
    .attr('data-bootstro-step', '5');
$('#words')
    .addClass('bootstro')
    .attr('data-bootstro-title', 'Words')
    .attr('data-bootstro-content', 'With the market in position you can choose which word to start at that location. We guess the word length, but you should adjust it.')
    .attr('data-bootstro-placement', 'top')
    .attr('data-bootstro-step', '6');
$('#container')
    .addClass('bootstro')
    .attr('data-bootstro-title', 'Adjusting')
    .attr('data-bootstro-content', 'Drag the start and ends of words. Green words are ones that you annotated. The orange word is the currenctly selected one. If any white words exist, they are references we provide to make life eaiser.')
    .attr('data-bootstro-placement', 'bottom')
    .attr('data-bootstro-step', '7');
$('#d3')
    .addClass('bootstro')
    .attr('data-bootstro-title', 'Verifying')
    .attr('data-bootstro-content', 'You should adjust the word boundaries by dragging them into the correct position on the diagram. The audio will automatically play. You can replay by clicking here or by using the keyboard shortcuts in red.')
    .attr('data-bootstro-placement', 'top')
...
$('#annotations')
    ...
    .attr('data-bootstro-content', 'Sometimes we have reference annotation available. You can select any references here. This includes any previous work you have done. They appear in white on the audio. Your annotations are in green and your selected annotation is in orange. The white annotations cannot be changed.')
    .attr('data-bootstro-placement', 'top')
    .attr('data-bootstro-step', '10');
$('#replace-reference')
    .addClass('bootstro')
    .attr('data-bootstro-title', 'References')
    .attr('data-bootstro-content', 'You can replace your annotation with the reference one or use the reference to fill in missing parts of your annotations.')
    .attr('data-bootstro-placement', 'top')
    .attr('data-bootstro-step', '11');
$('#seek')
    .addClass('bootstro')
    .attr('data-bootstro-title', 'Longer clips')
    .attr('data-bootstro-content', 'You see 4 seconds of audio each time. You can navigate forward and backward by 2 seconds or 4 seconds in the longer audio clip.')
    .attr('data-bootstro-placement', 'top')
    .attr('data-bootstro-step', '12');
$('#save-and-seek')
    .addClass('bootstro')
    .attr('data-bootstro-title', 'Saving while navigating')
    .attr('data-bootstro-content', 'You will usually annotate and then move on to the next segment. These buttons move you but also save your work each time.')
    .attr('data-bootstro-placement', 'top')
...
$('#intro').click(() => {
    ...
    bootstro.start('.bootstro', {
        finishButton: '<button class="btn btn-mini btn-warning bootstro-finish-btn"><i class="icon-ok"></i>Exit tutorial</button>',
    });
});
