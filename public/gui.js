...
function keyboardShortcutsOn() {
    ...
    $(document).bind('keydown', 'y', function() {$("#play-selection").click()})
    ...
    $(document).bind('keydown', 'b', function() {$("#back-save-2-sec").click()})
    $(document).bind('keydown', 'f', function() {$("#forward-save-2-sec").click()})
    $(document).bind('keydown', 'n', function() {$("#forward-2-sec").click()})
    $(document).bind('keydown', 'p', function() {$("#back-2-sec").click()})
    $(document).bind('keydown', 's', function() {$("#submit").click()})
    $(document).bind('keydown', 'r', function() {$("#fill-with-reference").click()})
}
...
function setup(buffer) {
    ...
    sourceNode.onended = function () { audioIsPlaying -= 1; redraw(null); }
...
function timeToPosition(time) { return (time / (endS-startS)) * canvas.width }
function timeInBufferToPosition(time) { return (time / sourceNode.buffer.duration) * canvas.width }
function positionToTime(position) { return position * (endS-startS) / canvas.width }
function positionToTimeInBuffer(position) { return position * sourceNode.buffer.duration / canvas.width }
function positionToAbsoluteTime(position) { return startS + position * (endS-startS) / canvas.width }
...
function levenshteinAlignment(iWords, i, jWords, j) {
    if (i >= iWords.length)
        return {distance: Math.abs(jWords.length - j)}
    if (j >= jWords.length)
        return {distance: Math.abs(iWords.length - i)}
...
function find_annotation(id) {
    ...
    if(typeof id === 'string') {
        return _.find(other_annotations_by_worker[id.split(':')[0]],
                      a => a.id == id);
    }
}
...
function shuffleSelection() {
    let workerAnnotations = svgAnnotations.selectAll('g').sort(function (a,b) {
        // TODO Selection
        return d3.ascending(find_annotation(a).lastClickTimestamp, find_annotation(b).lastClickTimestamp);
    })[0][0];
...
function playAnnotation(annotation) {
    ...
    setup(buffers[bufferKind]);
    if(annotation.end != null)
        play(positionToTimeInBuffer(annotation.start),
             positionToTimeInBuffer(annotation.end) - positionToTimeInBuffer(annotation.start))
    else
        play(positionToTimeInBuffer(annotation.start), defaultPlayLength())
}
...
function submit(next) {
    ...
    $.ajax({
        ...
        data: JSON.stringify({segment: segment,
                              ...
                              annotations:
                              _.map(_.filter(annotations, a => !_.isUndefined(a.start)),
                                    function(a)
                                    { return {start: a.start,
                                              end: a.end,
...
$("#transcript-input").keypress(function( event ) {
    if ( event.which == 13 ) {
        ...
        updateWordsWithAnnotations(_.filter(_.split($("#transcript-input").val(), ' '), a => a !== ""))
...
$('#fill-with-reference').click(event => {
    ...
    if(referenceAnnotations) {
        ...
        _.map(mergedAnnotations, a => { if(a) { updateWord(a) }});
        current_reference_annotation = undefined;
        $('.annotation').each((i, a) => {
            if($(a).text() == 'none') {
                $(a).removeClass('btn-default').addClass('btn-success')
...
$('#back-save-2-sec').click(function(event) {
    console.log('bs2')
    submit(a => reload(movieName + ':' + ('' + (startS-2)).padStart(5, '0') + ':' + ('' + (endS-2)).padStart(5, '0')))
})
...
function render_other_annotations(worker) {
    ...
    if(reference_annotations) {
        _.forEach(other_annotations_by_worker,
                  as => _.forEach(as, removeAnnotation));
        $('.annotation').each((i, a) => {
            if($(a).text() == worker) {
                $(a).removeClass('btn-default').addClass('btn-success')
            } else {
...
function reload(segmentName) {
    ...
    $('#annotations').empty()
    $('#annotations')
        .append($('<button type="button" class="annotation btn btn-success">')
                .text('none')
    ...
    if(segmentName) {
        ...
        window.history.pushState($.url().param(),
                                 "Audio annotation", "/gui.html?" + $.param(param));
    }
...
$('#container-wrapper').addClass('bootstro')
...
$('#toggle-speed-div').addClass('bootstro')
...
$('#transcript-panel').addClass('bootstro')
...
$('#edit-transcript').addClass('bootstro')
...
$('#delete-selection').addClass('bootstro')
...
$('#annotations').addClass('bootstro')
    .attr('data-bootstro-title', "References")
    .attr('data-bootstro-content', "Sometimes we have reference annotation available, you can select any references here. This includes any previous work you have done. They appear in white on the audio.")
    .attr('data-bootstro-placement', "top")
    .attr('data-bootstro-step', '10')

$('#replace-with-reference-annotation').addClass('bootstro')
    .attr('data-bootstro-title', "References")
...
$('#save-and-seek').addClass('bootstro')
...
$('#submit-button').addClass('bootstro')
